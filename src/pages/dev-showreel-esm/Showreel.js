...
const SPACE_ID = '7c765cb5-15c3-4876-a496-4141adc6bd99'
...
const Showreel = () => {
  ...
  // initialise smplr.js via ES module
  useEffect(() => {
    import('https://dev.smplrspace.com/lib/smplr.mjs').then(smplr => {
      renderSpace({ smplr, spaceId: SPACE_ID, preview: false })
    })
  }, [])

  const renderSpace = ({ smplr, spaceId, ...options }) => {
    setSpaceReady(null)
    spaceRef.current && spaceRef.current.remove()
    spaceRef.current = new smplr.Space({
      spaceId,
      clientToken: 'pub_95f93f1b121844a7a7040ea9b6246e36',
